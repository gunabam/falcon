from falcon.Reaction import ion_lib, sic_lib, cm_lib, cc_lib, cr_lib, re_lib
from falcon.Molecule import Molecule
from falcon.Logger import logging
...
class FragmentNodeFunc:
    ...
    def branch_propagation_1(self, nodes):
        # keep all overlapping nodes
        # but only run cr for node with most future branches
    ...
    def branch_propagation_2(self, nodes):
        # keep all overlapping nodes
        # but disable all reactions except for simple_inductive_cleavage
    ...
    def sort_nodes_by_future_branches(self, nodes, cascade_check=True):
        ...
        nodes = sorted(
            nodes, key=lambda k: (k["future_branches"], k["hash_id"]), reverse=True
        )
        return nodes
...
class FragmentNetwork:
    def __init__(
        ...
        self.NodeFunc = FragmentNodeFunc(min_mass)
        self.ring_bonds = set()
    ...
    def ionization(self):
        ...
        nodes = ion_lib.run_reaction_set(self.parent_molecule)
        # filter nodes
    ...
    def fragmentation(self, nodes_to_fragment, is_final_round):
        ...
        logging.info("New potential nodes to add: {}".format(len(nodes_to_add)))
        nodes_to_delete = set()
    ...
    def nl_check(self, node):
        ...
        in_nl = any(
            [self.graph[e1][e2]["has_neutral"] for e1, e2 in self.graph.in_edges(node)]
        )
        if in_nl == True:
            return True
        else:
    ...
    def ring_check(self, node):
        ...
        if rxn_type == "charge_migration":
            ...
            successor_check = any(
                [self.graph.nodes[n]["ring_break"] for n in self.graph.successors(node)]
            )
