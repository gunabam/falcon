...
from falcon.Molecule import Molecule, validate_mol
from falcon.Resonance import resonance_enum
from falcon.utils import reactions_df, terms
...
class Reaction:
    ...
    def run_reaction(self, reactant, ring_bonds, do_cascade_check, debug):
        fragment_list = []
        # setup reactants
    ...
    def process_product_set(self, mol, reactant, ring_bonds, do_cascade_check, debug):
        # parse mol
        charged = None
        neutral = []
...
class ReactionLibrary:
    ...
    def run_reaction_set(
        self, molecule, ring_bonds=set(), do_cascade_check=False, debug=False
    ):
        nodes_to_add = []
        for rxn in self.reactions:
            nodes = rxn.run_reaction(molecule, ring_bonds, do_cascade_check, debug)
    ...
    def estimate_future_branches(self, molecule, cascade_check=True):
        ...
        for rxn in self.reactions:
            ...
            if cascade_check:
                map_patts = [set(atom_to_map_dict[a] for a in p) for p in patts]
                count += len([p for p in map_patts if len(p & reaction_centre) > 0])
            else:
                count += len(patts)
        return count
